# W-C--- 4.0 Database Decryptor

W-C--- 4.0 (Windows) 本地数据库解密工具。从运行中的W-C---进程内存提取加密密钥，解密所有 SQLCipher 4 加密数据库，并提供实时消息监听。

## 原理

W-C--- 4.0 使用 SQLCipher 4 加密本地数据库：
- **加密算法**: AES-256-CBC + HMAC-SHA512
- **KDF**: PBKDF2-HMAC-SHA512, 256,000 iterations
- **页面大小**: 4096 bytes, reserve = 80 (IV 16 + HMAC 64)
- **每个数据库有独立的 salt 和 enc_key**

WCDB (W-C---的 SQLCipher 封装) 会在进程内存中缓存派生后的 raw key，格式为 `x'<64hex_enc_key><32hex_salt>'`。本工具通过扫描进程内存中的这种模式，匹配数据库文件的 salt，并通过 HMAC 验证来提取正确的密钥。
...
```json
{
    "db_dir": "D:\\xwechat_files\\你的W-C---ID\\db_storage",
...
`db_dir` 路径可以在 W-C---设置 → 文件管理 中找到。

### 2. 提取密钥

确保W-C---正在运行，以**管理员权限**运行：
...
将W-C---数据查询能力接入 [Claude Code](https://claude.ai/claude-code)，让 AI 直接读取你的W-C---消息。
...
|------|------|
| `config.py` | 配置加载器 |
| `find_all_keys.py` | 从W-C---进程内存提取所有数据库密钥 |
| `decrypt_db.py` | 全量解密所有数据库 |
| `mcp_server.py` | MCP Server，让 Claude AI 查询W-C---数据 |
...
W-C---使用 SQLite WAL 模式，WAL 文件是**预分配固定大小** (4MB)。检测变化时：
- 不能用文件大小 (永远不变)
- 使用 mtime 检测写入
...
本工具仅用于学习和研究目的，用于解密**自己的**W-C---数据。请遵守相关法律法规，不要用于未经授权的数据访问。
